...
This file is the canonical spec; what ships in this codebase is a subset.

**Implemented**
- Color tokens (canvas, ink, primary `#f54e00`, timeline pastels, hairlines, semantic) in `src/app/globals.css`.
- Typography utilities (`text-display-mega/lg/md/sm`, `text-title-md/sm`, `text-body-md/tracked/sm`, `text-caption`, `text-caption-uppercase`, `text-code`, `text-button`, `text-nav-link`).
- Component re-skin: Button, Card, Input, Badge, Dialog, Sheet, Alert Dialog, Tabs, Sidebar; primitives use Cursor tokens, no drop shadows, hairline-only depth.
- Code surfaces: `.code-pane` / `.code-pane-editable` utilities applied to every feature route (json-viewer, sql-placeholder, properties-converter, record-protobuf, totp-generator, enhance-prompt). JetBrains Mono on `--canvas-soft`. Caret + selection in Cursor Orange on editable surfaces.
- Prism token color overrides bound to the Cursor palette.
- Dark mode derived from Cursor light tokens (no published Cursor dark spec).

**Substituted**
- CursorGothic is licensed; Inter is the substitute display + body family. Cormorant Garamond is fully removed.

**Deferred (no host surface in this dev-tools app)**
- Hero band, IDE mockup card, pricing tier, marketing CTA band, agent timeline UI. Tokens land; components are not built.
